...
void place_tokens(square board[NUM_ROWS][NUM_COLUMNS], player players[], int numPlayers){
    ...
    for(i=0;i<4;i++)
    { //These loops ensure that each player places all their tokens on the board
        for(j=0;j<numPlayers;j++)
        {
            printf("Player %d please choose a square.\n", j);
            scanf("%d", &squareSelected); //Asks where the player wants to place their next token
            
            if(squareSelected==minTokens && squareSelected!=players[j].col)
            { //The following instructions add a token to a square.
